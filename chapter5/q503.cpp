...
class String::Iterator : public boost::iterator_facade<
    String::Iterator, String, boost::forward_traversal_tag>
{
    private:
        char *char_ptr;
        explicit Iterator(char* const ptr) :
            char_ptr(ptr) {}
        friend const String::Iterator String::begin();
        friend const String::Iterator String::end();
        
    public:
        friend class String::ConstIterator;
        Iterator() : char_ptr(nullptr) {}
        explicit Iterator(const String &s) :
            char_ptr(s.p_str) {}
        bool operator!=(char *cmp_ptr)
        {
            return this->char_ptr != cmp_ptr;
        }
        char& operator*()
        {
            return *char_ptr;
        ...
        {
            return char_ptr == in_iter.char_ptr;
        }
};

const String::Iterator String::begin()
{
    return String::Iterator(this->p_str);
}

const String::Iterator String::end()
{
    return String::Iterator(this->p_str + this->size());
}

class String::ConstIterator : public boost::iterator_facade<
    String::Iterator, String, boost::forward_traversal_tag>
{
    private:
        char const *char_ptr;
    ...
    public:
        ConstIterator() : char_ptr(nullptr) {}
        ConstIterator(const String &s) :
            char_ptr(s.p_str) {}
        ConstIterator(char* ptr) :
            char_ptr(ptr) {}
        ConstIterator(const String::Iterator &iter) :
            char_ptr(iter.char_ptr) {}
        bool operator!=(char *cmp_ptr)
        {
            return this->char_ptr != cmp_ptr;
        }
...
int main()
{
    ...
    String::ConstIterator sci = si;
    for (String::Iterator i = s.begin(); i != s.end(); ++i) {
        ++(*i);
    }
    for (String::ConstIterator i = s.begin(); i != s.end(); ++i) {
        std::cout << *i;
    }
    std::cout << "\n";
}
